...
% =========================================================================
%  >>>  TROCAR AQUI PARA MUDAR DE AERONAVE  <<<
%  Descomente o bloco da aeronave desejada e comente o outro.
% =========================================================================

% ── AH-1S Cobra ──────────────────────────────────────────────────────────
heli            = jsondecode(fileread('heli_params.json'));
aeronave_prefix = 'AH1S';
...
% ── AlphaOne ─────────────────────────────────────────────────────────────
% heli            = jsondecode(fileread('heli_params_alphaone.json'));
% aeronave_prefix = 'AlphaOne';
% fase3_VDM       = true;    % F3 = cruzeiro na VDM, F4 = loiter na VAM
% tempo_loiter    = 20;      % [min]
% casos = [ ...
%     1,    0,  300,  1000 ; ...
...
for k = 1 : size(casos, 1)
    ...
    output_folder = sprintf('results/%s/CASO%d', aeronave_prefix, caso);
    fprintf('\n======================================================\n');
    fprintf(' %s – CASO %d  (vento=%d kt | dist=%d NM | Vc_sub=%d fpm)\n', ...
            aeronave_prefix, caso, V_vento, distancia_3, Vc_sub_fpm);
    fprintf('======================================================\n');
    ...
    W_antes = W_atual;
    [potencias, W_atual] = Calcular_Fase_PesoMedio(W_atual, inf, Zp_2, dT, heli, Vy_2, Vc_sub_fpm, tempo_2);
    ...
    W_antes = W_atual;
    [potencias, W_atual] = Calcular_Fase_PesoMedio(W_atual, inf, 5000, dT, heli, V_f3, 0, tempo_3);
    ...
    W_antes = W_atual;
    [potencias, W_atual] = Calcular_Fase_PesoMedio(W_atual, inf, 5000, dT, heli, V_f4, 0, tempo_loiter);
    ...
    W_antes = W_atual;
    [potencias, W_atual] = Calcular_Fase_PesoMedio(W_atual, inf, Zp_5, dT, heli, Vy_5, -1000, tempo_5);
    ...
    W_antes = W_atual;
    [potencias, W_atual] = Calcular_Fase_PesoMedio(W_atual, 6, 0, dT, heli, 0, 0, 5);
